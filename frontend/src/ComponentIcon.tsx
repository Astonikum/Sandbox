...
export function ComponentIcon({ kind }: { kind: Kind }) {
  return (
    ...
    >
      {kind === "rect" ? (
        <rect x="6" y="5" width="20" height="18" fill="#cececa" />
      ) : kind === "circle" ? (
        <circle cx="16" cy="14" r="10" fill="#cececa" />
      ) : kind === "surface" ? (
        <>
        ...
        <>
          <circle cx="16" cy="14" r="11" fill="#eee" />
        ...
        <>
          ...
          {kind === "force" && <circle cx="5" cy="22" r="2" fill="black" />}
        </>
      )}
    </svg>
  );
